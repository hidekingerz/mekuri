...
**mekuri** は、圧縮ファイル（ZIP/RAR等）内の画像を見開き表示で閲覧するためのデスクトップアプリケーションである。Tauri v2（Rust + React）で構成される。
...
- FR-M5: 選択中のフォルダ内のアーカイブファイルをファイルリストとして一覧表示する
- FR-M6: アーカイブファイル（ZIP/RAR）をアイコンで識別表示する
- FR-M7: アーカイブファイルをクリックするとビューワーウィンドウを開く
...
| Home | 最初の見開きへ | ビューワー |
| End | 最後の見開きへ | ビューワー |
| マウスホイール | ページ送り（スロットル200ms） | ビューワー |
| Alt+↑ | 次の兄弟アーカイブへ | ビューワー |
| Alt+↓ | 前の兄弟アーカイブへ | ビューワー |
