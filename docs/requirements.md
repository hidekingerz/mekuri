...
**mekuri** は、圧縮ファイル（ZIP/RAR等）内の画像および PDF ファイルを見開き表示で閲覧するためのデスクトップアプリケーションである。Deno Desktop（`Deno.BrowserWindow` + Deno プロセス、TypeScript バックエンド + React フロントエンド）で構成される。
...
以下の設定は自前の `Store` クラス（`settings.json`）により自動保存・復元される。
...
- NFR-2: Deno Desktop 対応プラットフォーム（macOS 等）で動作する
