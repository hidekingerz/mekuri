...
5. **ITERATE**
   ...
   - **VERIFY 成功** → 変更を1コミットにまとめてコミットする（コミットメッセージは英語、簡潔に）。`loop/MEMORY.md` の Done に何を達成したか（commit ハッシュ含む）を追記し Open を更新したうえで、**`loop/MEMORY.md` も同じコミットに含める**（作業ツリーを dirty なまま残さない）。
