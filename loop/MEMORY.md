...
- [next] マイルストーン2は完了。次はマイルストーン3（フロント API 層の配線替え）に着手する。`src/api/`（`archive.ts`/`directory.ts`/`favorites.ts`/`settings.ts`/`store.ts`）を Tauri `invoke` から Deno バインディング呼び出しへ切替える。まず `src/api/` 各ファイルと現状の Tauri 呼び出し（どのコマンド名・引数・戻り値型を使っているか）、および `deno-app/bindings/` の現状（空のはず）を読む。`backend/mod.ts` が公開する関数（`readDirectory`/`readFileBase64`/`searchDirectory`/`listImages`/`analyzeContents`/`extractNestedArchive`/`getImageBase64`/拡張子判定）と React 側が必要とするインターフェースの対応表を作り、`deno-app/bindings/` にプロセス内バインディングを用意してから `src/api/` を1ファイルずつ配線替えする（1周1ファイル目安）。注意: RULES により `src/components/` 本体は無改変、変更は `src/api/` のみ。`favorites`/`settings`/`store` はマイルストーン5（tauri-plugin-store 相当）に依存するため、先に着手可能な `directory.ts`/`archive.ts` から進めるのが安全。stable deno で検証可能な範囲（型・ロジック）に留め、`Deno.BrowserWindow` 依存（マイルストーン4）は避ける。
