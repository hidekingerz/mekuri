 ...
 * 呼んでいた。Deno Desktop への移行では、webview 側で `fetch("/__invoke")`（HTTP
 * transport）でメインプロセスへ送る Tauri 互換シグネチャの shim
 * （`deno-app/frontend/invoke.ts`）を使う。
 *
...
export type { InvokeArgs } from "../../deno-app/frontend/invoke.ts";
